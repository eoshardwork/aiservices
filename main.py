...
from utils import (
    delete_db, doc_loader, summary_prompt_creator, doc_to_final_summary,
)
...
def main():
    ...
    input_method = st.radio("Select input method",
                            ('Upload a document', 'Enter a YouTube URL'))
    ...
    st.markdown('[Author email](mailto:eoshardwork@gmail.com)')
    ...
    st.sidebar.markdown(
        '# Made by: [eoshardwork](https://github.com/eoshardwork)')
    st.sidebar.markdown(
        '# Git link: [AIservices](https://github.com/eoshardwork/aiservices)')
    ...
    if st.button('Summarize (click once and wait)'):
        if input_method == 'Upload a document':
            process_summarize_button(
                uploaded_file, api_key, use_gpt_4)

        else:
            doc = transcript_loader(youtube_url)
            process_summarize_button(
                doc, api_key, use_gpt_4, file=False)
...
def process_summarize_button(file_or_transcript, api_key, use_gpt_4, file=True):
    ...
    with st.spinner("Summarizing... please wait..."):
        ...
        summary = doc_to_final_summary(
            doc, initial_prompt_list, final_prompt_list, api_key, use_gpt_4)
